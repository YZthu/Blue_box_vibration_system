...
def myplot(number):
    ...
    foot_step = [[0*x for x in range(PLOT_NUM)] for j in range(len(sensor_ip_list))]
...
#vb_wt = []
        for ii in range(number):
            if not queue[ii].empty():
                print("ii is %d, and queue length is %d"%(ii, queue[ii].qsize()))
                data = queue[ii].get()
...
#print(foot_step[ii])
                ...
                if (ii ==1) and (len(tmp_peak) > 0):
                    ...
                    if (position < 100) or (position + 300 > tmplen):
                        ...
                        vb = raw_data[ii][600*PLOT_NUM -new_len - 300 + position -1000 :600*PLOT_NUM-new_len -300 + position +1000]
                        vb = vb - np.mean(vb)
...
#   print("all time is %f"%(t2-t1))
            if ii == (number -1):
                half_step = [row[0:int(PLOT_NUM/2)] for row in foot_step[:]]
                low_half_step = [row[int(PLOT_NUM/2)+1:PLOT_NUM] for row in foot_step[:]]
                
                his_list = np.sum(half_step, axis=1)
                new_list = np.sum(low_half_step, axis=1)
                print(his_list)
                print(new_list)
                print('###')
                ax[ii +1].cla()
                ax[ii +1].set_ylim([-22500, 22500])
            ...
            if ii == (number -1):
                ...
                ax[ii +2].bar( x1, his_list, width=wid, label='past', fc='y')
                ax[ii +2].bar( x2, new_list, width=wid, label='now', tick_label= name_list, fc='b')
                plt.legend()
                plt.pause(0.001)
